  ...
  const useAsTemplate = async (project: Project) => {
    try {
      const result = await window.electron.invoke("project-library:use-template", {
        ...
        projectPath: project.localPath || `${project.owner}/${project.name}`,
      });
      if (result.success) {
        // Navigate to the new project or show success message
        console.log("Project created from template:", result.appId);
      }
    } catch (error) {
