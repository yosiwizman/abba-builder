...
export function registerCIHandlersV2() {
  ...
  // Configure provider
  ipcMain.handle('ci:configure-provider', async (_, config: {
    providerId: string;
    type: CIProviderType;
    auth: {
    ...
    try {
      logger.info(`${LOG_PREFIX} Configuring provider: ${config.providerId} (${config.type})`);
      ...
      const success = await providerManager.registerProvider(config.providerId, providerConfig);
      
      if (success) {
        logger.info(`${LOG_PREFIX} Provider configured successfully: ${config.providerId}`);
        return { success: true, message: 'Provider configured successfully' };
      } else {
        logger.error(`${LOG_PREFIX} Failed to configure provider: ${config.providerId}`);
  ...
  ipcMain.handle('ci:get-providers', async () => {
    try {
      ...
      const providerList = Array.from(providers.entries()).map(([id, provider]) => ({
        id,
        name: provider.name,
        type: provider.type,
      ...
      logger.info(`${LOG_PREFIX} Returning ${providerList.length} configured providers`);
      return providerList;
    } catch (error) {
  ...
  ipcMain.handle('ci:set-active-provider', async (_, providerId: string) => {
    try {
      ...
      logger.info(`${LOG_PREFIX} Set active provider to: ${providerId} - ${success ? 'Success' : 'Failed'}`);
      return { success };
    } catch (error) {
  ...
  // Get builds
  ipcMain.handle('ci:get-builds', async (_, options?: {
    branch?: string;
    limit?: number;
    offset?: number;
  }) => {
    try {
      ...
      const dashboardBuilds = builds.map(build => ({
        ...
        duration: build.duration ? Math.floor(build.duration / 1000) : undefined,
        commitHash: build.commit,
        author: build.author,
        message: build.commitMessage
      }));
      
      logger.info(`${LOG_PREFIX} Returning ${dashboardBuilds.length} builds from provider`);
  ...
  // Get deployments
  ipcMain.handle('ci:get-deployments', async (_, options?: {
    environment?: string;
    limit?: number;
  }) => {
    try {
      ...
      logger.info(`${LOG_PREFIX} Returning ${dashboardDeployments.length} deployments from provider`);
  ...
  // Get statistics
  ipcMain.handle('ci:get-statistics', async (_, options?: {
    ...
    try {
      ...
      if (!provider || !provider.isAuthenticated()) {
        logger.warn(`${LOG_PREFIX} No active provider, returning mock statistics`);
  ...
  ipcMain.handle('ci:trigger-build', async (_, { project, branch }) => {
    try {
      ...
      if (!provider || !provider.isAuthenticated()) {
        logger.warn(`${LOG_PREFIX} No active provider, simulating build trigger`);
        return {
          success: true,
  ...
  ipcMain.handle('ci:get-build-details', async (_, buildId: string) => {
    try {
      ...
      if (!provider || !provider.isAuthenticated()) {
        logger.warn(`${LOG_PREFIX} No active provider, returning mock build details`);
        return {
      ...
      return {
        build: {
          ...
          duration: buildDetails.duration ? Math.floor(buildDetails.duration / 1000) : undefined,
        ...
        steps: buildDetails.steps?.map(step => ({
          ...
          duration: step.duration ? Math.floor(step.duration / 1000) : undefined
        })),
  ...
  // Trigger deployment
  ipcMain.handle('ci:trigger-deployment', async (_, { environment, version, buildId }) => {
    try {
      const provider = providerManager.getActiveProvider();
      
      if (!provider || !provider.isAuthenticated()) {
        logger.warn(`${LOG_PREFIX} No active provider, simulating deployment`);
        return {
          success: true,
      ...
      logger.info(`${LOG_PREFIX} Triggering deployment to ${environment} with version ${version}`);
  ...
  logger.info(`${LOG_PREFIX} Enhanced CI/CD IPC handlers registered successfully`);
}
