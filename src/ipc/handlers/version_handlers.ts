...
export function registerVersionHandlers() {
  // Alias for new API: version:list
  handle("version:list", async (_, { appId }: { appId: number }) => {
  ...
  async function revertCore({ appId, previousVersionId }: RevertVersionParams): Promise<RevertVersionResponse> {
      ...
      if (app.neonProjectId && app.neonDevelopmentBranchId) {
        ...
        if (version && version.neonDbTimestamp) {
          try {
            ...
            // Update all versions newer than this DB timestamp
            await db
              .update(versions)
            ...
            // Do not throw to allow finishing
          }
  ...
  handle(
    ...
    async (
      ...
      return revertCore({ appId, previousVersionId });
    },
  );

  // Alias for new API: version:rollback
  handle(
    "version:rollback",
    async (
      _,
      ...
      return revertCore({ appId, previousVersionId: versionId });
    },
  );

  // Create checkpoint (commit all changes) alias for new API
  handle(
    "version:create-checkpoint",
    async (
      _,
      { appId, name }: { appId: number; name?: string },
    ): Promise<{ commit: string }> => {
      const app = await db.query.apps.findFirst({ where: eq(apps.id, appId) });
      if (!app) throw new Error("App not found");
      const appPath = getDyadAppPath(app.path);
      // Stage all changes and commit
      await import("../utils/git_utils").then(async ({ gitAddAll, gitCommit }) => {
        await gitAddAll({ path: appPath });
        const commit = await gitCommit({ path: appPath, message: name || `Checkpoint ${new Date().toISOString()}` });
        return commit;
      });
      return { commit: await git.resolveRef({ fs, dir: appPath, ref: "HEAD" }) };
    },
  );
