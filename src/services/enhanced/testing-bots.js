...
class AbbaTestingBots {
    ...
    if (!this.playwright) {
      try {
        ...
        try {
          ...
          console.warn('[AbbaTestingBots] Playwright not available:', err?.message);
        }
      }
    }
  ...
  async createTestingBot(appType, generatedCode, requirements = {}) {
    ...
    return {
      webAppBot: appType === 'web' ? await this.createWebTestBot(generatedCode, requirements) : null,
      desktopBot: appType === 'desktop' ? await this.createDesktopTestBot(generatedCode, requirements) : null,
      extensionBot: appType === 'extension' ? await this.createExtensionTestBot(generatedCode, requirements) : null,
      mobileBot: appType === 'mobile' ? await this.createMobileTestBot(generatedCode, requirements) : null
    };
  }
  ...
  async createExtensionTestBot(generatedCode) {
    ...
    return {
      runManifestValidation: async () => ({ success: true, notes: 'Extension test stub' })
    };
  }
  ...
  generateTestScenarios(code, requirements) {
    ...
    scenarios.push({
      ...
      expectations: ['page responds', 'no console errors', 'UI updates correctly']
    });
    ...
    if (this.hasForms(code)) {
      scenarios.push({
        ...
        priority: 'high',
        steps: [
          { action: 'type', selector: 'input[type="text"], input[type="email"], input:not([type])', text: 'test@example.com' },
          { action: 'type', selector: 'input[type="password"]', text: 'testPassword123' },
          { action: 'click', selector: 'button[type="submit"], input[type="submit"], form button, form [type="submit"]' }
        ],
        expectations: ['form submits', 'validation works', 'appropriate feedback']
      });
    }
    ...
    if (requirements?.features?.includes('search')) {
      scenarios.push({
        ...
        priority: 'medium',
        steps: [
          { action: 'type', selector: 'input[type="search"], input[placeholder*="search" i]', text: 'hello world' },
          { action: 'wait', duration: 800 },
          { action: 'click', selector: 'button:has-text("Search"), [role="button"]:has-text("Search")' }
        ],
  ...
  hasForms(code) {
    ...
    if (typeof code === 'object') {
      return Object.values(code).some((v) => typeof v === 'string' && /<form|<input|<textarea|<select/i.test(v));
    }
    return false;
  }
    ...
    return {
      web: {
        ...
        scrolling: {
          ...
          overshoot: 0.20, // 20% chance of scrolling too far
          backtrack: 0.30 // 30% chance of scrolling back up
        }
      },
      desktop: {
        windowInteraction: {
          ...
          multipleWindows: 0.40 // 40% chance of opening multiple instances
        }
      }
    };
  }
...
class WebTestingBot {
  ...
  async deployForTesting() {
    ...
    const tempRoot = await fsp.mkdtemp(path.default.join(os.default.tmpdir(), 'abba-test-'));
    ...
    if (indexHtml && typeof this.code === 'string') {
      await fsp.writeFile(path.default.join(tempRoot, 'index.html'), indexHtml, 'utf8');
    }
    ...
    const server = http.default.createServer(async (req, res) => {
      try {
        ...
        const filePath = path.default.join(serveDir, decodeURIComponent(safePath));
  ...
  async launchHumanLikeBrowser() {
    ...
    if (this.runtimes.playwright?.chromium) {
      ...
      this.page = await this.browser.newPage({ viewport: this.getRandomViewport() });
    ...
    throw new Error('No browser automation runtime available (Playwright/Puppeteer not found)');
  }
  ...
  async executeHumanLikeStep(step) {
    switch (step.action) {
      ...
      case 'type':
        ...
        break;
      case 'scroll':
        await this.humanLikeScroll(step.direction || 'down', step.amount || 400);
        break;
      case 'wait':
      ...
      case 'navigate':
        ...
        break;
      case 'resize':
        await this.page.setViewportSize({ width: step.width || 1280, height: step.height || 800 });
        break;
    }
  }

  async humanLikeClick(selector) {
    const element = await this.page.locator ? this.page.locator(selector).first() : null;

    if (element && (await element.count()) > 0) {
      // Use Playwright locator if available
      await this.wait(this.randomDelayMs(100, 400));
      const doubleClick = Math.random() < (this.humanBehavior?.clickPatterns?.doubleClicks || 0);
      if (doubleClick) {
  ...
  async humanLikeType(selector, text) {
    ...
    const locator = this.page.locator ? this.page.locator(selector).first() : null;
    ...
    if (locator && (await locator.count()) > 0) {
      ...
      for (let i = 0; i < words.length; i++) {
        ...
        if (Math.random() < (this.humanBehavior?.typingPatterns?.typos || 0)) {
          ...
          if (Math.random() < (this.humanBehavior?.typingPatterns?.corrections || 0)) {
            await this.wait(this.randomDelayMs(150, 500));
            for (let k = 0; k < typo.length; k++) await this.page.keyboard.press('Backspace');
            await typeWithDelay((ch, opts) => locator.type(ch, opts), word, 10, 50);
          }
        } else {
          await typeWithDelay((ch, opts) => locator.type(ch, opts), word);
        }
        if (i < words.length - 1) await locator.type(' ');
        if (Math.random() < 0.15) await this.wait(this.randomDelayMs(300, 1200));
      }
      return;
    }
    ...
    if (handle) {
      ...
      for (let i = 0; i < words.length; i++) {
        ...
        if (Math.random() < (this.humanBehavior?.typingPatterns?.typos || 0)) {
          const typo = this.introduceTypo(word);
          await this.page.type(selector, typo, { delay: this.randomDelayMs(20, 80) });
          if (Math.random() < (this.humanBehavior?.typingPatterns?.corrections || 0)) {
            await this.wait(this.randomDelayMs(150, 500));
            for (let k = 0; k < typo.length; k++) await this.page.keyboard.press('Backspace');
            await this.page.type(selector, word, { delay: this.randomDelayMs(10, 60) });
          }
        } else {
          await this.page.type(selector, word, { delay: this.randomDelayMs(20, 80) });
        }
  ...
  async measurePerformance() {
    try {
      const perf = await this.page.evaluate(() => {
        ...
        return {
          domContentLoaded: (t.domContentLoadedEventEnd || 0) - (t.navigationStart || 0),
  ...
  generateTestReport(results) {
    ...
    return {
      ...
      criticalIssues: results.filter((r) => !r.success && r.scenario?.includes('Basic')).length,
      detailedResults: results,
  ...
  generateRecommendations(results) {
    const recs = [];
    const hasLayout = results.some((r) => r.scenario?.includes('Responsive') && !r.success);
    const hasErrors = results.some((r) => (r.issues || []).some((i) => String(i).includes('validation')));
    if (hasLayout) recs.push('Improve responsive layout breakpoints and test at 375/768/1920 widths.');
    if (hasErrors) recs.push('Review console/network errors and add user-friendly error states.');
    if (!recs.length) recs.push('Looks good! Add more domain-specific tests for deeper coverage.');
    return recs;
  }
  ...
  introduceTypo(word) {
    ...
    switch (typoType) {
      ...
      case 'wrong':
        ...
        if (nearKeys[char]) {
          const wrongChar = nearKeys[char][this.randomInt(0, nearKeys[char].length - 1)];
