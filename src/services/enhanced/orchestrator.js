...
class EnhancedOrchestrator {
  constructor(config = {}) {
    this.config = {
      ...
      budgetLimit: config.budgetLimit || 10.0,
  ...
  async initializeEnhancedServices() {
    if (this.config.enableTemplates) {
      this.services.templateMatcher =
        await this.loadService("template-matcher");
    }
  ...
  async generateDesktopApp(request, options = {}, context) {
    ...
    if (
      !this.services.desktopFrameworkManager ||
      !this.services.desktopCodeGenerator
    ) {
    ...
    const prompts =
      this.services.desktopFrameworkManager.generateFrameworkSpecificPrompts(
        framework,
        request,
        context,
      );
  ...
  async generateWebApp(request, options = {}, context) {
    ...
    if (this.services.promptOptimizer) {
      optimizedPrompt = await this.services.promptOptimizer.optimize(
        request,
        "web-app",
      );
    }
    ...
    if (this.services.templateMatcher) {
      template = await this.services.templateMatcher.findBestTemplate(
        request,
        "web",
      );
    }
  ...
  async compareFrameworks(request, requirements = {}) {
    ...
    return this.services.desktopFrameworkManager.compareFrameworks(
      requirements,
    );
  }
  ...
  async runAutomatedTests(appType, generatedCode, requirements) {
    ...
    try {
      ...
      if (appType === "web" && testingBot.webAppBot) {
        ...
        console.log(
          `Test results: ${testResults.overallSuccess ? "PASSED" : "FAILED"} (${testResults.successRate}% success rate)`,
        );
        return testResults;
      }
  ...
  async testGeneratedApp(appType, code, requirements = {}) {
    ...
    const bot = await this.services.testingBots.createTestingBot(
      appType,
      code,
      requirements,
    );
