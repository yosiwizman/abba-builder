...
class TestDrivenRefinement {
  ...
  async refineBasedOnTestResults(code, testResults, requirements = "") {
    ...
    while (
      !this.allTestsPass(currentTests) &&
  ...
  calculateSeverity(failure) {
    ...
    if (
      failure.test?.includes("critical") ||
      failure.test?.includes("required")
    ) {
      return "high";
    }
    if (
  ...
  prioritizeFixes(analysis) {
    ...
    analysis.failures
      ...
      .forEach((f) =>
        priority.push({ type: "null-reference", item: f, priority: 2 }),
      );
    ...
    analysis.failures
      ...
      .forEach((f) =>
        priority.push({ type: "type-error", item: f, priority: 3 }),
      );
    ...
    analysis.failures
      ...
      .forEach((f) =>
        priority.push({ type: "assertion", item: f, priority: 4 }),
      );
  ...
  async generateFixes(code, analysis, requirements) {
    ...
    for (const item of analysis.priority.slice(0, 5)) {
      // Fix top 5 issues
  ...
  async fixAssertion(code, failure, requirements) {
    ...
    if (expectation) {
      ...
      const patch = await this.generateAssertionFix(
        code,
        expectation,
        requirements,
      );
  ...
  extractVariableName(errorMessage) {
    ...
    const match = errorMessage.match(
      /['"]?(\w+)['"]?\s+is\s+(undefined|not defined)/,
    );
  ...
  async generateAssertionFix(code, expectation, requirements) {
    ...
    if (verb === "be" || verb === "equal") {
      ...
      patch.pattern = new RegExp(
        `(let|const|var)\\s+${subject}\\s*=\\s*[^;]+;`,
      );
      ...
      for (const change of fix.changes) {
        if (change.pattern && change.replacement) {
          ...
          if (change.conditional) {
            ...
            const testCode = fixedCode.replace(
              change.pattern,
              change.replacement,
            );
  ...
  getStatistics() {
    ...
    if (this.refinementHistory.length > 0) {
      ...
      stats.successfulFixes = this.refinementHistory.filter(
        (r) => r.testResults && this.allTestsPass(r.testResults),
      ).length;
