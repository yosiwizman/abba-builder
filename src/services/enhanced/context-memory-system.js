...
class ContextMemorySystem {
  ...
  migrateProfile(profile) {
    ...
    if (!profile.learning) profile.learning = { skills: [], interests: [], improvements: [] };
  ...
  recordInteraction(userId, interaction) {
    ...
    if (interaction.duration) {
      const totalTime = profile.statistics.averageBuildTime * (profile.statistics.totalProjects - 1);
      profile.statistics.averageBuildTime = (totalTime + interaction.duration) / profile.statistics.totalProjects;
    }
    ...
    if (interaction.type === 'project' && interaction.success) {
      profile.projects.push({
        ...
        name: interaction.metadata?.projectName || `Project ${profile.projects.length + 1}`,
  ...
  learnFromInteraction(profile, interaction) {
    ...
    if (interaction.metadata?.colors && Array.isArray(interaction.metadata.colors)) {
      profile.preferences.colors.push(...interaction.metadata.colors);
      profile.preferences.colors = this.getTopItems(profile.preferences.colors, 5);
    }
    ...
    if (interaction.metadata?.framework) {
      ...
      profile.preferences.frameworks = this.getTopItems(profile.preferences.frameworks, 3);
    ...
    if (interaction.request) {
      ...
      profile.learning.skills = [...new Set([...profile.learning.skills, ...skills])];
      
      const interests = this.extractInterests(interaction.request);
      profile.learning.interests = [...new Set([...profile.learning.interests, ...interests])];
    }
  }

  extractSkills(text) {
    const skillKeywords = [
      'react', 'vue', 'angular', 'nextjs', 'nodejs', 'python', 'java',
      'typescript', 'javascript', 'html', 'css', 'tailwind', 'bootstrap',
      'database', 'api', 'rest', 'graphql', 'authentication', 'deployment'
    ];
  ...
  enhanceRequest(userId, originalRequest) {
    ...
    const context = {
      ...
      successRate: profile.statistics.totalProjects > 0 
  ...
  buildEnhancedPrompt(context) {
    ...
    if (context.userPreferences.colors.length > 0) {
      preferences.push(`Preferred colors: ${context.userPreferences.colors.join(', ')}`);
    }
    
    if (context.userPreferences.frameworks.length > 0) {
      preferences.push(`Preferred frameworks: ${context.userPreferences.frameworks.join(', ')}`);
    }
    
    preferences.push(`Complexity preference: ${context.userPreferences.complexity}`);
    ...
    if (context.interests.length > 0) {
      preferences.push(`User interests: ${context.interests.slice(0, 3).join(', ')}`);
    }
    
    // Only add preferences if they're relevant to the request
    if (preferences.length > 0 && !context.request.toLowerCase().includes('ignore preferences')) {
      enhanced += `\n\n[User Context]\n${preferences.join('\n')}`;
    }
  ...
  getInsights(userId = this.getUserId()) {
    ...
    const successRate = profile.statistics.totalProjects > 0 
      ? (profile.statistics.successfulProjects / profile.statistics.totalProjects * 100).toFixed(1)
      : 'N/A';
    ...
    const insights = {
      summary: {
        ...
        activeProjects: profile.projects.filter(p => p.status === 'active').length
      },
      preferences: {
  ...
  generateRecommendations(profile) {
    ...
    if (profile.statistics.successfulProjects / profile.statistics.totalProjects < 0.7) {
      recommendations.push({
        type: 'complexity',
        message: 'Consider starting with simpler projects to improve success rate',
        action: 'setComplexity:simple'
      });
    }
    
    // Recommend new frameworks based on skills
    const unusedFrameworks = ['react', 'vue', 'angular', 'nextjs']
      .filter(f => !profile.preferences.frameworks.includes(f));
    ...
    if (profile.learning.interests.includes('e-commerce') && 
  ...
  exportUserData(userId = this.getUserId()) {
    const profile = this.getUserProfile(userId);
    const exportPath = path.join(this.memoryPath, `export_${userId}_${Date.now()}.json`);
  ...
  clearUserData(userId = this.getUserId()) {
    ...
    if (fs.existsSync(profilePath)) {
      ...
      const backupPath = path.join(this.memoryPath, `backup_${userId}_${Date.now()}.json`);
