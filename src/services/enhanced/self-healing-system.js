...
class SelfHealingSystem {
  ...
  async startMonitoring(deployment) {
    ...
    console.log(`🩺 Started monitoring ${deployment.projectName || deployment.id}`);
  ...
  async performHealthCheck(deployment) {
    ...
    try {
      ...
      if (!health.healthy) {
        ...
        console.log(`⚠️ Health check failed for ${deployment.projectName || deployment.id}: ${health.issue}`);
        
        // Attempt healing if threshold reached
        if (monitor.consecutiveFailures >= 2 && monitor.healingAttempts < this.maxHealingAttempts) {
        ...
        if (monitor.consecutiveFailures > 0) {
          console.log(`✅ ${deployment.projectName || deployment.id} recovered!`);
      ...
      console.error(`Health check error for ${deployment.projectName || deployment.id}:`, error);
      monitor.failures++;
      monitor.consecutiveFailures++;
  ...
  async checkHealth(deployment) {
    ...
    if (deployment.platforms?.database) {
      try {
        const dbHealth = await this.checkDatabaseHealth(deployment.platforms.database);
        health.checks.database = dbHealth;
        
        if (!dbHealth.healthy) {
          health.healthy = false;
          health.issue = health.issue ? `${health.issue}, Database unhealthy` : 'Database unhealthy';
        }
      } catch (error) {
  ...
  async attemptHealing(deployment, health) {
    console.log(`🔧 Attempting to heal ${deployment.projectName || deployment.id}...`);
    ...
    try {
      ...
      if (health.issue === 'Connection failed' || health.issue === 'Deployment not found (404)') {
        // Try to redeploy
      ...
      } else if (health.issue === 'High memory usage' || health.issue === 'High CPU usage') {
      ...
      console.log(`✅ Healing successful for ${deployment.projectName || deployment.id}`);
      
    } catch (error) {
      healing.success = false;
      healing.error = error.message;
      console.error(`❌ Healing failed for ${deployment.projectName || deployment.id}:`, error);
    }
  ...
  async triggerRedeploy(deployment) {
    console.log(`Triggering redeploy for ${deployment.projectName || deployment.id}`);
  ...
  async resetDatabaseConnection(deployment) {
    console.log(`Resetting database for ${deployment.projectName || deployment.id}`);
  ...
  async scaleDeployment(deployment, direction = 'up') {
    console.log(`Scaling ${direction} ${deployment.projectName || deployment.id}`);
  ...
  getMonitoringStatus() {
    ...
    this.monitors.forEach((monitor, id) => {
      ...
      const availability = monitor.checks > 0 
        ? ((monitor.checks - monitor.failures) / monitor.checks * 100).toFixed(2)
        : 100;
      ...
      status.push({
        ...
        lastCheck: monitor.lastCheck ? new Date(monitor.lastCheck).toISOString() : 'Never'
      });
    });
