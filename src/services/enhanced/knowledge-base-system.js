...
class KnowledgeBaseSystem {
  ...
  saveKnowledge() {
    ...
    fs.writeFileSync(
      bugsPath,
      JSON.stringify(Array.from(this.bugs.values()), null, 2),
    );
    ...
    fs.writeFileSync(
      compatPath,
      JSON.stringify(Array.from(this.compatibility.values()), null, 2),
    );
  }
  ...
  learnFromSuccess(successData) {
    ...
    solutions.forEach((solution) => {
      ...
      if (
        !existingSolution ||
        existingSolution.confidence < solution.confidence
      ) {
  ...
  getRecommendations(requirements, techStack = []) {
    ...
    recommendations.patterns.sort(
      (a, b) => b.confidence * b.similarity - a.confidence * a.similarity,
    );
  ...
  countComponents(code) {
    const componentRegex =
      /class\s+\w+\s+extends\s+(React\.)?Component|function\s+[A-Z]\w+|const\s+[A-Z]\w+\s*=/g;
  ...
  findSimilarFailure(newFailure) {
    ...
    this.failures.forEach((failure) => {
      if (
        failure.errorType === newFailure.errorType &&
  ...
  extractBug(code, error) {
    ...
    bug.affectedTech = imports.filter(
      (imp) =>
        imp.includes("react") ||
        imp.includes("vue") ||
  ...
  generateFailureSuggestion(failure) {
    ...
    return (
      suggestions[failure.errorType] || "Review similar successful patterns"
    );
  }
  ...
  getStatistics() {
    const stats = {
      ...
      successfulPatterns: Array.from(this.patterns.values()).filter(
        (p) => p.confidence > 0.8,
      ).length,
