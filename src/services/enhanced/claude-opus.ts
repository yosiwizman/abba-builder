...
import { getAPIKeys } from "../../../config/secrets";
...
export class ClaudeOpusService {
  ...
  constructor(config?: ClaudeOpusConfig) {
    ...
    this.apiKey = config?.apiKey || apiKeys.anthropic || "";
    ...
    this.STREAMING_TIMEOUT_MS = Number(
      process.env.CLAUDE_STREAMING_TIMEOUT || 300000,
    ); // 5 minutes
    ...
    if (this.apiKey) {
      ...
      this.logger.warn(
        "Claude API key not found, will attempt OpenAI fallback if available",
      );
  ...
  async generateWithFullContext(
    ...
    const preferStreaming = Boolean(
      process.env.CLAUDE_FORCE_STREAMING === "true" ||
        (projectContext?.totalTokens || 0) > 10000,
    );

    console.log(
      `🔍 Claude.generateWithFullContext -> preferStreaming=${preferStreaming}, modelsToTry=${modelsToTry.join(" -> ")}`,
    );
    ...
    for (const model of modelsToTry) {
      for (let attempt = 0; attempt < this.MAX_RETRIES; attempt++) {
        try {
          if (preferStreaming) {
            const res = await this.callStreaming(
              model,
              contextualPrompt,
  ...
  async generateWithThinking(
    ...
    for (const stage of stages) {
      ...
      if (preferStreaming || stage === "implement" || stage === "optimize") {
        result = await this.generateWithFullContext(
          stagePrompt,
          context,
          30000,
        );
  ...
  private buildContextualPrompt(prompt: string, context: any): string {
    ...
    const truncatedContext =
      contextStr.length > maxContextChars
...
Always think step-by-step and explain your reasoning when implementing complex solutions.`;
  ...
  async generateWithStreaming(
    ...
    const modelsToTry = [this.currentModel, this.fallbackModel];
    console.log(
      `🔍 Claude.generateWithStreaming(timeout=${timeoutMs}) models=${modelsToTry.join(" -> ")}`,
    );
  ...
  private async callStreaming(
    ...
    try {
      ...
      console.log(
        `✅ Claude ${model} streamed response in ${duration}ms, ${content.length} chars`,
      );
      return {
        success: true,
        content,
      ...
      const duration = Date.now() - start;
      console.warn(
        `Streaming failed for ${model} after ${duration}ms:`,
        err?.message || err,
      );
      throw err;
    }
