...
class IntegrationHub {
  ...
  async buildAndDeploy(userId, request, options = {}) {
    ...
    try {
      ...
      console.log(`  Found ${recommendations.patterns.length} matching patterns`);
      ...
      console.log(`  Built using ${buildResult.strategy} strategy in ${buildResult.attempts} attempts`);
      
      // STEP 4: Test and refine if needed
      console.log('\n🧪 Step 4: Testing and refining...');
      let finalCode = buildResult.code;
      
      if (buildResult.testResults && !this.allTestsPass(buildResult.testResults)) {
      ...
      console.log(`  Total time: ${(buildSession.duration / 1000).toFixed(1)}s`);
  ...
  getSuccessRateString() {
    ...
    const rate = (this.successRate.successful / this.successRate.total * 100).toFixed(1);
  ...
  getAverageBuildTime() {
    ...
    const totalTime = this.buildHistory.reduce((sum, b) => sum + (b.duration || 0), 0);
