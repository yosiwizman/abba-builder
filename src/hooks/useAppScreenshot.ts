...
export function useAppScreenshot() {
  ...
  const captureScreenshot = useCallback(async (appId: number): Promise<string | null> => {
    setIsCapturing(true);
    ...
    try {
      ...
      await new Promise(resolve => {
        if (iframe.contentDocument?.readyState === 'complete') {
          ...
          iframe.addEventListener('load', () => resolve(void 0), { once: true });
        }
      });
  ...
  const captureAndSaveScreenshot = useCallback(async (appId: number) => {
