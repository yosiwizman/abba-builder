...
async function runVisualTests() {
  ...
  for (const target of testTargets) {
    ...
    try {
      ...
      if (testResult.visual?.results) {
        ...
        console.log(`\n📊 Results:`);
        console.log(`  - Total viewports tested: ${visualData.summary?.totalTests || 0}`);
        ...
        if (visualData.comparisons && visualData.comparisons.length > 0) {
          ...
          visualData.comparisons.forEach(comp => {
            ...
            console.log(`    ${status} ${comp.viewport.name}: ${comp.difference.toFixed(2)}% diff`);
          });
        }
      }
  ...
  const reportPath = join(process.cwd(), 'visual-results', `report_${Date.now()}.json`);
