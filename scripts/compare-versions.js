...
async function compareVersions() {
  ...
  console.log('Compare visual changes across different versions of your app.\n');
  ...
  if (options.versions.length === 0) {
    ...
    if (availableVersions.length < 2) {
      ...
      console.log('💡 Tip: Capture baselines at different times or after changes');
      process.exit(1);
    }
    
    // Use the last two versions
    options.versions = availableVersions.slice(-2);
    console.log(`📦 Comparing last two versions: ${options.versions.join(' vs ')}\n`);
    ...
    if (currentBaseline) {
      ...
      console.error('❌ Error: Need two versions to compare or a current baseline');
      process.exit(1);
    }
  }
...
async function performComparison(visualTesting, appId, versions) {
  ...
  for (let i = 1; i < versions.length; i++) {
    ...
    // Find baseline files
    const prevPath = prevVersion === 'current' 
      ? join(baselinePath, `${appId}_baseline.json`)
      : join(archivePath, `${appId}_${prevVersion}`, `${appId}_baseline.json`);
      
    const currPath = currVersion === 'current'
      ? join(baselinePath, `${appId}_baseline.json`)
      : join(archivePath, `${appId}_${currVersion}`, `${appId}_baseline.json`);

    if (!fs.existsSync(prevPath) || !fs.existsSync(currPath)) {
      console.warn(`⚠️  Cannot find baselines for ${prevVersion} → ${currVersion}`);
      continue;
    }
    ...
    const comparison = {
      ...
      viewportChanges: analyzeViewportChanges(prevData.screenshots, currData.screenshots)
    };

    results.comparisons.push(comparison);
    results.summary.totalChanges += Math.abs(comparison.screenshotChanges.difference);
  }
    ...
    if (!prevMap.has(key)) {
      ...
      if (Math.abs(prevSize - currSize) > prevSize * 0.1) {
        changes.push({ 
          type: 'modified', 
          viewport: key, 
          sizeChange: ((currSize - prevSize) / prevSize * 100).toFixed(1) + '%'
        });
      }
    }
  });
...
function displayComparisonResults(results) {
  ...
  results.comparisons.forEach(comp => {
    console.log(`\n📦 ${comp.from} → ${comp.to}`);
    console.log(`   Time: ${new Date(comp.fromTimestamp).toLocaleDateString()} → ${new Date(comp.toTimestamp).toLocaleDateString()}`);
    console.log(`   Screenshots: ${comp.screenshotChanges.previous} → ${comp.screenshotChanges.current} (${comp.screenshotChanges.difference >= 0 ? '+' : ''}${comp.screenshotChanges.difference})`);
    ...
    if (comp.viewportChanges.length > 0) {
      ...
      comp.viewportChanges.forEach(change => {
        ...
        console.log(`     ${icon} ${change.viewport}${change.sizeChange ? ` (${change.sizeChange})` : ''}`);
      });
    }
  });
...
function provideRecommendations(results) {
  ...
  results.comparisons.forEach(comp => {
    if (comp.screenshotChanges.difference > 0) {
      recommendations.push(`📱 New viewports added in ${comp.to} - ensure all are tested`);
    } else if (comp.screenshotChanges.difference < 0) {
      recommendations.push(`⚠️  Viewports removed in ${comp.to} - verify this is intentional`);
    }

    const modifiedViewports = comp.viewportChanges.filter(c => c.type === 'modified');
    if (modifiedViewports.length > 0) {
      recommendations.push(`🔄 ${modifiedViewports.length} viewports show visual changes - review for regressions`);
    }
  });
...
function listAvailableVersions() {
  ...
  if (apps.size === 0) {
    ...
    console.log('💡 Tip: Run "npm run test:visual:baseline" to capture baselines first.');
    return;
  }
  ...
  apps.forEach((versions, appId) => {
    ...
    versions.sort().forEach(version => {
      const timestamp = version === 'current' ? 'Latest' : new Date(parseInt(version)).toLocaleDateString();
 ...
 */
function showHelp() {
  console.log('Usage: npm run test:visual:compare [options] [app-id] [version1] [version2]\n');
  console.log('Options:');
  console.log('  --list, -l              List all available app versions');
  console.log('  --threshold, -t <value> Set difference threshold (default: 0.1)');
