...
async function runIntegrationTest() {
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        fs.existsSync(testConfig.resultsPath)
      ) {
    ...
    try {
      ...
      if (allMethodsExist) {
        ...
        const scenarios = await integration.generateTestScenarios(
          testCode,
          "html",
        );
    ...
    results.overallSuccess =
      Object.values(results).filter((v) => v === true).length >= 3;
  } catch (error) {
  ...
  console.log(
    `Directory Creation:  ${results.directoryCreation ? "✅ PASS" : "❌ FAIL"}`,
  );
  console.log(
    `Configuration:       ${results.configurationSetup ? "✅ PASS" : "❌ FAIL"}`,
  );
  console.log(
    `Integration Layer:   ${results.integrationLayer ? "✅ PASS" : "❌ FAIL"}`,
  );
  console.log("\n" + "-".repeat(60));
  console.log(
  ...
  if (results.overallSuccess) {
    ...
    console.log("\n📝 Next steps:");
    console.log(
      '   1. Use "npm run test:visual:baseline" to capture baselines',
    );
